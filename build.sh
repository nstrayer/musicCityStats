...
function show_help {
   ...
   echo "-f   convert '/Users/Nick/Dropbox/vandy/musicCityStats/blog/_knitr/<filename>.Rmd' to md in _posts"
...
if [ "$convert" = true ] ; then
    ...
    if [ "$overwrite_md" = true ] ; then
        Rscript -e "source('/Users/Nick/Dropbox/vandy/musicCityStats/blog/_knitr/render_post.R'); KnitPost(overwrite=TRUE)"
    else
        Rscript -e "source('/Users/Nick/Dropbox/vandy/musicCityStats/blog/_knitr/render_post.R'); KnitPost()"
    fi
fi
...
if [ "$convert_file" = true ] ; then
   ...
   if [ -f $rmdfile ]; then
      Rscript -e "source('/Users/Nick/Dropbox/vandy/musicCityStats/blog/_knitr/render_post.R'); KnitPost(bashwd='$bashpwd',convert_file='$rmdfile')"
   else
      show_help
      exit 0
   fi
...
if [ "$buildsite" = true ] ; then
    ...
    jekyll build -s /Users/Nick/Dropbox/vandy/musicCityStats/blog -d /var/www/blog
fi
